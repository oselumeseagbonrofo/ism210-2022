...
const HelloWorld6: React.FC<Props> = ({ name = "Idemudia" }) => {
    ...
    const setUserIdToFetch = (event: any) => {
        if (event.target.value != '') { //let us ensure that user fetch does not
            //happen when id is wiped
    ...
    return (
        <div className='flex flex-col space-y-3'>
            <p>Hello {name}. This illustrates hooking into a componentDidUpdate using a remote call. HelloWorld6</p>
            <p>
                <input type="text" placeholder="Write a
            user id here to fetch ..." name="user_id"
